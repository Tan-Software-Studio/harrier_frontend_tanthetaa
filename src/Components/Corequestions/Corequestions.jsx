...
import { Backdrop, Button, Chip, Fade, FormControl, FormControlLabel, FormLabel, Input, InputBase, InputLabel, MenuItem, Modal, OutlinedInput, TextareaAutosize } from '@mui/material'
...
const Corequestions = () => {
    ...
    const [desiredWorkingArrangementsSelected, setDesiredWorkingArrangementsSelected] = useState([])
    ...
    const DesiredRoleSelectedMap = desiredRoleSelected && desiredRoleSelected.map((d, i) => (d.id))
    const WillingBasedSelectedMap = willingBasedSelected && willingBasedSelected.map((d, i) => (d.id))
    const desiredCountrySelectedMap = desiredCountrySelected && desiredCountrySelected.map((d, i) => (d.id))

    const desiredWorkingArrangementsMap = desiredWorkingArrangementsSelected && desiredWorkingArrangementsSelected.map((d, i) => (d.id))
    const customerTypeSelectedMap = customerTypeSelected && customerTypeSelected.map((d, i) => (d.id))
    const languagesSelectedMap = languagesSelected && languagesSelected.map((d, i) => (d.id))
    const disabilitySelectedMap = disabilitySelected && disabilitySelected.map((d, i) => (d.id))
    const legalTechToolsSelectedMap = legalTechToolsSelected && legalTechToolsSelected.map((d, i) => (d.id))
    const techToolsSelectedMap = techToolsSelected && techToolsSelected.map((d, i) => (d.id))
    const qualificationSelectedMap = qualificationSelected && qualificationSelected.map((d, i) => (d.id))
    ...
    const [higherEduSelected, setHighrEduSelected] = useState({ id: "", title: "" });
    const [freeSchoolSelected, setFreeSchoolSelected] = useState({ id: "", title: "" });
    const [indicateReligionSelected, setIndicateReligionSelected] = useState({ id: "", title: "" });
    const [employmentVisaSelected, setEmploymentVisaSelected] = useState({ id: "", title: "" });
    const [earnerOccupationSelected, setEarnerOccupationSelected] = useState({ id: "", title: "" });
    ...
    const [candidatesJobStatusSelected, setCandidatesJobStatusSelected] = useState({ id: "", title: "" });

    const [type, setType] = useState();
    const [sendFreelance, setSendFreelance] = useState({ id: "", title: "" });
    const [sendFreelanceFuture, setSendFreelanceFuture] = useState({ id: "", title: "" });
    const [currentSalarySymbol, setCureentSalarySymbol] = useState({ id: "", symbol: "" });

    const [annualBonusSymbol, setAnnualBonusSymbol] = useState({ id: "", symbol: "" });
    const [desiredSalarySymbol, setDesiredSalarySymbol] = useState({ id: "", symbol: "" });
    const [desiredAnnualBonusSymbol, setDesiredAnnualBonusSymbol] = useState({ id: "", symbol: "" });
    const [desiredDayRateSymbol, setDesiredDayRateSymbol] = useState({ id: "", symbol: "" });


    const [sexualOrientationsOptions, setSexualOrientationsOptions] = useState([]);
    const [sexualOrientations, setSexualOrientations] = useState({ id: "", symbol: "" });

    const [mainEarnerOccupationsOptions, setMainEarnerOccupationsOptions] = useState([]);
    const [mainEarnerOccupations, setMainEarnerOccupations] = useState({ id: "", symbol: "" });

    const [sexSelected, setSexSelected] = useState({ id: "", symbol: "" });
    const [genderIdentitySelected, setGenderIdentitySelected] = useState({ id: "", title: "" });

    const [dealSizeSymbol, setDealSizeSymbol] = useState({ id: "", symbol: "" });
    const [saleQuotaSymbol, setSaleQuotaSymbol] = useState({ id: "", symbol: "" });
    ...
    const firstStep = async () => {
        ...
        await axiosInstance
            .post("v1/candidate/form/step/one/validate",
                formData,
            )
            .then((res) => {
                const data = Decrypt(res.data.data)
    ...
    const secondStep = async () => {
        ...
        await axiosInstance
            .post("v1/candidate/form/step/two/validate",
                formData,
            )
            .then((res) => {
                const data = Decrypt(res.data.data)
    ...
    const thirdStep = async () => {
        ...
        await axiosInstance
            .post("v1/candidate/form/step/three/validate",
                formData,
            )
            .then((res) => {
                const data = Decrypt(res.data.data)
                        ...
                        <div className="row">
                            <div className="col-lg-3 ">
                                <div className="left-tabs ">
                                    ...
                                    <button
                                        className={`${toggleState === 2 ? "tabs tabs-active " : "tabs"} mt-20`}
                                    // onClick={() => toggleTab(2)}
                                    >
                                    ...
                                    <button
                                        className={`${toggleState === 3 ? "tabs tabs-active" : "tabs"} mt-20`}
                                    // onClick={() => toggleTab(3)}
                                    >
                                        <span>  Diversity & Inclusion </span>
                                    </button>
                                    <button
                                        className={`${toggleState === 4 ? "tabs tabs-active" : "tabs"} mt-20`}
                                    // onClick={() => toggleTab(4)}
                                    >
                                    ...
                                    <div className="content-tabs">
                                        ...
                                        >
                                            ...
                                            <div className='form-start px-60'>
                                                <div className="row">
                                                        ...
                                                        <div className='w-100'>
                                                            <FormControl variant="standard">
                                                                <InputLabel shrink htmlFor="uname">
                                                                    What is your First Name? <span className="text-red"> * </span>
                                                                </InputLabel>
                                                                <TextField
                                                                    id="firstname"
                                                                    // error
                                                        ...
                                                        <div className='w-100'>
                                                            <FormControl variant="standard">
                                                                <InputLabel shrink htmlFor="lastname">
                                                                    What is your Last Name? <span className="text-red"> * </span>
                                                                </InputLabel>
                                                                <TextField id="lastname" placeholder='Enter last name' name='last_name' value={forminputData.last_name} onChange={inputData} />
                                                            </FormControl>
                                                        </div>
                                                    </div>
                                                    <div className="col-lg-4 col-md-6 mt-auto">
                                                        <div className='w-100'>
                                                            <FormControl variant="standard">
                                                                <InputLabel shrink htmlFor="enteremail">
                                                                    Enter your Email Address <span className="text-red"> * </span>
                                                                </InputLabel>
                                                                <TextField id="enteremail" placeholder='Enter email address' type="email" name='email' value={forminputData.email} onChange={inputData} />
                                                            </FormControl>
                                                        </div>
                                                    </div>
                                                    <div className="col-lg-4 col-md-6 mt-auto">
                                                        <div className='w-100'>
                                                            <FormControl variant="standard">
                                                                <InputLabel shrink htmlFor="phone-number">
                                                                    Enter your Phone Number <span className="text-red"> * </span>
                                                                </InputLabel>
                                                        ...
                                                        <div className='w-100'>
                                                            <FormControl variant="standard">
                                                                <InputLabel shrink htmlFor="job_title">
                                                                    What is your job title? <span className="text-red"> * </span>
                                                                </InputLabel>
                                                                <TextField id="job_title" placeholder='Enter job title' name='job_title' value={forminputData.job_title} onChange={inputData} />
                                                            </FormControl>
                                                        </div>
                                                    </div>
                                                    <div className="col-lg-4 col-md-6 mt-auto">
                                                        <div className='w-100'>
                                                            <FormControl variant="standard">
                                                                <InputLabel shrink htmlFor="enteremail">
                                                                    Who is your current employer? <span className="text-red"> * </span>
                                                                </InputLabel>
                                                                <TextField id="enteremail" placeholder='Enter current employer' name='employer' value={forminputData.employer} onChange={inputData} />
                                                            </FormControl>
                                                        </div>
                                                    </div>
                                                        ...
                                                        <div className='w-100'>
                                                            <FormControl variant="standard">
                                                                <InputLabel shrink htmlFor="companyurl">
                                                                    Enter current employer's URL. &#40; E.g. www.slaughterandmay.com &#41; <span className="text-red"> * </span>
                                                                </InputLabel>
                                                                <TextField id="companyurl" placeholder='E.g. www.slaughterandmay.com' name='current_company_url' value={forminputData.current_company_url} onChange={inputData} />
                                                            </FormControl>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div className="col-lg-4 col-md-6 mt-auto select-down-icon">
                                                        <div className='w-100'>
                                                            <FormControl variant="standard">
                                                                <InputLabel shrink htmlFor="uname">
                                                                    What is your Employer type? <span className="text-red"> * </span>
                                                                </InputLabel>
                                                            </FormControl>
                                                    ...
                                                    <div className="col-lg-4 col-md-6 mt-auto">
                                                        <FormControl variant="standard">
                                                            <InputLabel shrink htmlFor="lineManage">
                                                                Do you currently line manage anyone?
                                                                &#40; If no please type 0, if yes please enter specific number &#41;
                                                                <span className="text-red"> * </span>
                                                            </InputLabel>
                                                            <TextField
                                                                id="lineManage"
                                                    ...
                                                    <div className="col-lg-4 col-md-6 mt-auto select-down-icon  multiple-selects ">
                                                        <FormControl p={4} variant="standard">
                                                            <InputLabel shrink htmlFor="uname">
                                                                ...
                                                                &#40; Pick multiple if desired &#41; <span className="text-red"> * </span>
                                                            </InputLabel>
                                                    ...
                                                    <div className="col-lg-4 col-md-6 mt-auto select-down-icon">
                                                        <FormControl variant="standard">
                                                            <InputLabel shrink htmlFor="uname">
                                                                What country are you based in? <span className="text-red"> * </span>
                                                            </InputLabel>
                                                            {/* <Select
                                                    ...
                                                    <div className="col-lg-4 col-md-6 mt-auto select-down-icon  multiple-selects ">
                                                        <FormControl variant="standard">
                                                            <InputLabel shrink htmlFor="lineManage">
                                                                Are you open to working in other countries? &#40; You may pick none, one or multiple &#41;
                                                                <span className="text-red"> * </span>
                                                            </InputLabel>
                                                    ...
                                                    <div className="col-lg-4 col-md-6 mt-auto select-down-icon  multiple-selects">
                                                        <FormControl variant="standard">
                                                            <InputLabel shrink htmlFor="uname">
                                                                Where in your country are you willing to be based? &#40; E.g. for UK candidates: London, Manchester &#41; <span className="text-red"> * </span>
                                                            </InputLabel>
                                                            {/* <Select
                                                                isMulti
                                                                // name="global-tag-variant"
                                                            ...
                                                            <TextField id="desired_region" placeholder='E.g. for UK candidates: London, Manchester' name='desired_region' value={forminputData.desired_region} onChange={inputData} />
                                                    ...
                                                    <p className='mt-50 disclaimer'>
                                                        Only your desired salary will be shared with prospective employers, not your current base. Candidates will be able to request guest access to the
                                                        platform and browse the anonymised candidates database and see current salaries of their peers for their own research. For example, if you are
                                                        a Legal Technologist in London you can search by job title to see what other Legal Technologists in your region are being paid.
                                                    </p>

                                                    <p className='mt-20 disclaimer'>
                                                        Please choose your Job Seeking Status (Active, Passive, Very Passive, Closed) carefully. Employers are likely to use this as a key criterion in their search. Harrier will be in touch quarterly to make sure your profile is up to date. Definitions of the Status are as follows :
                                                    </p>
                                                    ...
                                                    <p className='disclaimer'>
                                                        Passive (open to receiving CV requests from Employers)
                                                    </p>
                                                    <p className='disclaimer'>
                                                        Very Passive (unlikely to accept CV requests unless the role is exceptional)
                                                    </p>
                                                    <p className='disclaimer'>
                                                        ...
                                                        <div className='symbol'>
                                                            <FormControl variant="standard">
                                                                <InputLabel shrink htmlFor="currentBaseSalary">
                                                                    What is your current annual base salary? <span className="text-red"> * </span>
                                                                </InputLabel>
                                                        ...
                                                        <div className='position-relative symbol'>
                                                            <FormControl variant="standard">
                                                                <InputLabel shrink htmlFor="commission">
                                                                    ...
                                                                    commission if relevant? <span className="text-red"> * </span>
                                                                </InputLabel>
                                                                <div className="d-flex border-radius-add" >
                                                                    <Autocomplete
                                                                        options={currenciesOptions}
                                                                    ...
                                                                    <TextField
                                                                        ...
                                                                        value={forminputData.current_bonus_or_commission}
                                                                        onChange={inputData}
                                                                        inputProps={{
                                                                            inputMode: 'numeric',
                                                                            min: 0,
                                                            ...
                                                            <p className='small-disclaimer'>Please state bonus amount rather than as a percentage of the base.</p>
                                                        </div>
                                                    </div>
                                                    <div className="col-lg-4 col-md-6 mt-auto">
                                                        <div className='symbol'>
                                                            <FormControl variant="standard">
                                                                <InputLabel shrink htmlFor="desiredsalary">
                                                                    What is your desired salary in your next role? <span className="text-red"> * </span>
                                                                </InputLabel>
                                                                <div className="d-flex border-radius-add">
                                                                    <Autocomplete
                                                                        options={currenciesOptions}
                                                        ...
                                                        <div className='symbol'>
                                                            <FormControl variant="standard">
                                                                <InputLabel shrink htmlFor="desiredbonus">
                                                                    What is your desired annual bonus,
                                                                    or commission if relevant? <span className="text-red"> * </span>
                                                                </InputLabel>
                                                                <div className="d-flex border-radius-add">
                                                                    <Autocomplete
                                                                        options={currenciesOptions}
                                                                    ...
                                                                    <TextField
                                                                        ...
                                                                        value={forminputData.desired_bonus_or_commission}
                                                                        onChange={inputData}
                                                                        inputProps={{
                                                                            inputMode: 'numeric',
                                                                            min: 0,
                                                    ...
                                                    <div className="col-lg-4 col-md-6 mt-auto">
                                                        <FormControl variant="standard">
                                                            <InputLabel shrink htmlFor="noticePeriod">
                                                                What is your current notice period?  &#40; Please give number of weeks &#41; <span className="text-red"> * </span>
                                                            </InputLabel>
                                                            <TextField
                                                                id="noticePeriod"
                                                    ...
                                                    <div className="col-lg-4 co-md-6 mt-auto select-down-icon">
                                                        <FormControl variant="standard">
                                                            <InputLabel shrink htmlFor="currentJob">
                                                                What is your current job-seeking status? <span className="text-red"> * </span>
                                                            </InputLabel>
                                                    ...
                                                    <div className="col-lg-4 col-md-6 mt-auto select-down-icon">
                                                        <FormControl variant="standard">
                                                            <InputLabel shrink htmlFor="currentWorking">
                                                                What are your current working arrangements? <span className="text-red"> * </span>
                                                            </InputLabel>
                                                    ...
                                                    <div className="col-lg-4 col-md-6 mt-auto select-down-icon  multiple-selects">
                                                        <FormControl variant="standard">
                                                            <InputLabel shrink htmlFor="currentJob">
                                                                What working arrangements would you consider? &#40; You may pick more than one option &#41; <span className="text-red"> * </span>
                                                            </InputLabel>
                                                    ...
                                                    <div className="col-lg-4 col-md-6 mt-auto select-down-icon">
                                                        <FormControl variant="standard">
                                                            <InputLabel shrink htmlFor="currentWork">
                                                                Do you current work freelance/as a contractor? <span className="text-red"> * </span>
                                                            </InputLabel>
                                                            {/* <Select
                                                    ...
                                                    <div className="col-lg-4 col-md-6 mt-auto select-down-icon">
                                                        <FormControl variant="standard">
                                                            <InputLabel shrink htmlFor="openFreelance">
                                                                Are you open to freelance work? <span className="text-red"> * </span>
                                                            </InputLabel>
                                                    ...
                                                    <div className="col-lg-4 col-md-6 mt-auto">
                                                        <div className='symbol'>
                                                            <FormControl variant="standard" className={sendFreelanceFuture?.id == 1 ? "" : "input-disabled"}>
                                                                <InputLabel shrink htmlFor="dayRate">
                                                                    What is your desired day rate? <span className="text-red"> * </span>
                                                                </InputLabel>

                                                                <div className="d-flex border-radius-add">
                                                                    <Autocomplete
                                                                        disabled={sendFreelanceFuture?.id == 1 ? false : true}
                                                                        options={currenciesOptions}
                                                                        getOptionLabel={
                                                                            (option) => (option.currency_code || "")
                                                                        }
                                                                    ...
                                                                    <TextField
                                                                        ...
                                                                        type="number" name='freelance_daily_rate'
                                                ...
                                                <div className='mt-50 text-end'>
                                                    ...
                                                    <button className="commom-sky-button" onClick={() => { firstStep() }} >Next</button>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        >
                                                ...
                                                <div className='px-60'>
                                                    <div className="row">
                                                            ...
                                                            <div className='radioBtn'>
                                                                <FormControl variant="standard">
                                                                    <InputLabel shrink htmlFor="lawDegree">
                                                                        Do you hold a law degree? <span className="text-red"> * </span>
                                                                    </InputLabel>
                                                                    <RadioGroup aria-labelledby=""
                                                                        defaultValue="female"
                                                                        name="radio-buttons-group" className='mt-11'>
                                                                        <div className='d-flex'>
                                                                            <FormControlLabel value="yes" control={<Radio />} label="Yes" onChange={oncheck} checked={lawlegal === "yes"} />
                                                                            <FormControlLabel value="no" control={<Radio />} label="No" onChange={oncheck} checked={lawlegal === "no"} />
                                                                        </div>
                                                                    </RadioGroup>
                                                                </FormControl>
                                                            </div>
                                                        ...
                                                        <div className="col-lg-4">
                                                            <div>
                                                                <div className='radioBtn'>
                                                                    <FormControl variant="standard" >
                                                                        <InputLabel shrink htmlFor="lawDegree">
                                                                            Are you a qualified lawyer? <span className="text-red"> * </span>
                                                                        </InputLabel>
                                                                        <RadioGroup aria-labelledby=""
                                                                            defaultValue="female"
                                                                            name="radio-buttons-group" className='mt-11'  >
                                                                            <div className='d-flex'>
                                                                                <FormControlLabel value="yes" control={<Radio />} label="Yes" onChange={qualifiedcheck} checked={qualifiedLawyer == "yes"} />
                                                                                <FormControlLabel value="no" control={<Radio />} label="No" onChange={qualifiedcheck} checked={qualifiedLawyer == "no"} />
                                                                            </div>
                                                                        </RadioGroup>
                                                                    </FormControl>
                                                                </div>
                                                        ...
                                                        <div className="col-lg-4 col-md-6 mt-auto">
                                                            ...
                                                            <div className={lawlegal == "yes" && qualifiedLawyer == "yes" ? "" : "input-disabled"}>
                                                                <FormControl variant="standard">
                                                                    <InputLabel shrink htmlFor="qualified">
                                                                        If so, in which jurisdiction are you qualified? <span className="text-red"> * </span>
                                                                    </InputLabel>
                                                                    <TextField id="qualified" placeholder='Enter qualified jurisdiction' name='jurisdiction' value={forminputData.jurisdiction} onChange={inputData} disabled={lawlegal == "yes" && qualifiedLawyer == "yes" ? false : true} />
                                                                </FormControl>
                                                            </div>
                                                        </div>
                                                        <div className="col-lg-4 col-md-6 mt-auto ">
                                                            <div className={lawlegal == "yes" && qualifiedLawyer == "yes" ? "" : "input-disabled"}>
                                                                <FormControl variant="standard" >
                                                                    <InputLabel shrink htmlFor="yearsPqe">
                                                                        Years PQE <span className="text-red"> * </span>
                                                                    </InputLabel>
                                                                    ...
                                                                    <TextField
                                                                        ...
                                                                        disabled={lawlegal == "yes" && qualifiedLawyer == "yes" ? false : true}
                                                                    />
                                                        ...
                                                        <div className="col-lg-4 col-md-6 mt-auto">
                                                            <div className={lawlegal == "yes" && qualifiedLawyer == "yes" ? "" : "input-disabled"}>
                                                                <FormControl variant="standard" >
                                                                    <InputLabel shrink htmlFor="yearsPqe">
                                                                        In which area of law do you/did you practise?  <span className="text-red"> * </span>
                                                                    </InputLabel>
                                                                    {/* <BootstrapInput id="area_of_law" placeholder='Enter area of practise' /> */}
                                                                    <TextField id="area_of_law" placeholder='Enter area of practise' name='area_of_law' value={forminputData.area_of_law} onChange={inputData} disabled={lawlegal == "yes" && qualifiedLawyer == "yes" ? false : true} />
                                                        ...
                                                        <div className="col-lg-4 col-md-6 mt-auto">
                                                            <div className={lawlegal == "yes" && qualifiedLawyer == "yes" ? "" : "input-disabled"}>
                                                                <FormControl variant="standard" >
                                                                    <InputLabel shrink htmlFor="specialistExperience">
                                                                        Do you have any specialist experience
                                                                        within your area of law? <span className="text-red"> * </span>
                                                                    </InputLabel>
                                                                    {/* <BootstrapInput id="yearsPqe" placeholder='Enter Experience ' /> */}
                                                                    <TextField id="legal_experience" placeholder='Enter Experience' name='legal_experience' value={forminputData.legal_experience} onChange={inputData} disabled={lawlegal == "yes" && qualifiedLawyer == "yes" ? false : true} />
                                                ...
                                                <div className='px-60'>
                                                    <div className="row">
                                                            ...
                                                            <div className='radioBtn'>
                                                                <FormControl variant="standard">
                                                                    <InputLabel shrink htmlFor="lawDegree">
                                                                        ...
                                                                        vendor/consultancy?  <span className="text-red"> * </span>
                                                                    </InputLabel>
                                                                    <RadioGroup aria-labelledby=""
                                                                        defaultValue="yes"
                                                                        name="radio-buttons-group"  >
                                                                        <div className='d-flex mt-11'>
                                                                            <FormControlLabel value="yes" control={<Radio />} label="Yes" onChange={oncheckd} />
                                                                            <FormControlLabel value="no" control={<Radio />} label="No" onChange={oncheckd} />
                                                                        </div>
                                                                    </RadioGroup>
                                                                </FormControl>
                                                            </div>
                                                        ...
                                                        <div className="col-lg-4 col-md-6 mt-auto select-down-icon multiple-selects">
                                                            ...
                                                            <div className={`${commercial === "no" ? "input-disabled" : ""}`}>
                                                                <div className='w-100'>
                                                                    <FormControl disabled={`${commercial === "no" ? true : ""}`}>
                                                        ...
                                                        <div className="col-lg-4 col-md-6 mt-auto">
                                                            <div className={`${commercial === "no" ? "input-disabled" : ""}`}>
                                                                <div className='symbol'>
                                                                    <FormControl variant="standard" disabled={`${commercial === "no" ? true : ""}`}>
                                                                        <InputLabel shrink htmlFor="yearsPqe">
                                                                            What is the average deal size with customers
                                                                            you close/manage?
                                                                            {commercial === "yes"
                                                                                ? <span className="text-red"> * </span>
                                                                                : ""}
                                            ...
                                            <div id='qualification'>
                                                <div className='bg-sky mt-50'>
                                                    <h3 className='font-20 px-60'>3. Tech/Other Qualifications</h3>
                                                </div>
                                                <div className='px-60'>
                                                    <div className="row">
                                                        <div className="col-lg-6 mt-auto multiple-selects">
                                                            ...
                                                            <FormControl variant="standard">
                                                                <InputLabel shrink htmlFor="legaltechtools">
                                                                    What LegalTech tools do you have experience in? We have some preset core LegalTech tools for you to select from where appropriate, but do type in any other tools and your familiarity. &#40; E.g. Betty Blocks - Beginner/Competent/Expert &#41;
                                                                    <span className="text-red"> * </span>
                                                                </InputLabel>
                                                                ...
                                                                <Autocomplete
                                                                    ...
                                                                    limitTags={1}
                                                                    multiple
                                                                    options={legalTechToolsOption.map((option) => option.title)}
                                                                    freeSolo
                                                                    onChange={(e, value) => {
                                                        ...
                                                        <div className="col-lg-6 mt-auto multiple-selects">
                                                            ...
                                                            <FormControl variant="standard">
                                                                <InputLabel shrink htmlFor="techExperience">
                                                                    Do you have any broader tech experience, such as coding or experience using business software such as Excel or a CRM platform? We have some preset options for you to select from where appropriate, but do type in any other tools/coding languages and your familiarity. &#40; E.g. Beginner/Competent/Expert  &#41;<span className="text-red"> * </span>
                                                                </InputLabel>
                                                                ...
                                                                <Autocomplete
                                                                    ...
                                                                    limitTags={1}
                                                                    multiple
                                                                    options={techToolsOption.map((option) => option.title)}
                                                                    freeSolo
                                                                    onChange={(e, value) => {
                                                        ...
                                                        <div className="col-lg-6 mt-auto multiple-selects">
                                                            ...
                                                            <FormControl variant="standard">
                                                                <InputLabel shrink htmlFor="qualification">
                                                                    Do you hold other relevant qualifications? &#40; E.g. JD, Scrum Master, Relativity Certified Administrator, PRINCE2 &#41;<span className="text-red"> * </span>
                                                                </InputLabel>
                                                                ...
                                                                <Autocomplete
                                                                    ...
                                                                    limitTags={1}
                                                                    multiple
                                                                    options={qualificationOption.map((option) => option.title)}
                                                                    freeSolo
                                                                    onChange={(e, value) => {
                                                        ...
                                                        <div className="col-lg-6 select-down-icon  multiple-selects">
                                                            ...
                                                            <FormControl variant="standard">
                                                                <InputLabel shrink htmlFor="language">
                                                                    What languages do you speak? &#40; For avoidance of doubt, please include English if applicable &#41;<span className="text-red"> * </span>
                                                                </InputLabel>
                                                        ...
                                                        <div className="col-lg-6 mt-auto">
                                                            ...
                                                            <FormControl variant="standard">
                                                                ...
                                                                /> */}
                                                                <TextareaAutosize className='w-100 p-2 h-100'
                                                                ...
                                                                {textAreaLength < 302 ?
                                                                    <p className='profile-word-count'>
                                                                        <span className='count'>
                                                                            {textAreaLength !== undefined ? textAreaLength : 0}
                                                                        </span> Out of
                                                                        <span className='count'>
                                                                            300
                                                                        </span>
                                            ...
                                            <div className='mt-50 text-end px-60'>
                                                <button className="commom-blue-button me-3" onClick={() => { setToggleState(toggleState - 1); }}>Previous</button>
                                                <button className="commom-sky-button" onClick={() => { secondStep() }} >Next</button>
                                        ...
                                        {/* =========== Third Tab ====== */}
                                        <div className={toggleState === 3 ? "contents active-contents" : "contents"}
                                        >
                                            <h3 className='question-title px-60'>Diversity & Inclusion</h3>
                                            <p className='border-btm'></p>
                                            <div className='form-start px-60'>
                                                <div className="row">
                                                    <p className='mt-50 disclaimer'>
                                                        Harrier Search Ltd is committed to helping clients build talented and diverse teams. We are also signatories to the <a target="blank" href='https://recruiter.racefairnesscommitment.com/'><span>
                                                            Recruitment Agency Race Fairness Commitment.</span></a>  To that end we would be grateful if you could complete this section of the questionnaire to help us in our monitoring and reporting on the diversity of candidates submitted to employers and their success rates. If you do not wish to share this information, each question has a ‘Prefer not to say’ option. We will never share this personal data, it is purely to help us do high-level monitoring and reporting.
                                                    </p>
                                                    <div className="col-lg-4 col-md-6 mt-auto select-down-icon multiple-selects">
                                                        <div className='w-100'>
                                                            <FormControl variant="standard">
                                                                <InputLabel shrink htmlFor="uname">
                                                                    Which cultural background do you most identify with? <span className="text-red"> * </span>
                                                                </InputLabel>
                                                                {/* <Select
                                                                    options={options}
                                                                    components={{ Input: CustomInput }}
                                                    ...
                                                    <div className="col-lg-4 col-md-6 mt-auto select-down-icon">
                                                        <div className='w-100'>
                                                            <FormControl variant="standard">
                                                                <InputLabel shrink htmlFor="uname">
                                                                    What is your sex? <span className="text-red"> * </span>
                                                                </InputLabel>
                                                                {/* <Select
                                                                    options={options}
                                                                    components={{ Input: CustomInput }}
                                                    ...
                                                    <div className="col-lg-4 col-md-6 mt-auto select-down-icon">
                                                        <div className='w-100'>
                                                            <FormControl variant="standard">
                                                                <InputLabel shrink htmlFor="uname">
                                                                    How would you describe your gender identity?<span className="text-red"> * </span>
                                                                </InputLabel>
                                                                {/* <Select
                                                                    options={options}
                                                                    components={{ Input: CustomInput }}
                                                        ...
                                                        <div className='w-100'>
                                                            <FormControl variant="standard">
                                                                <InputLabel shrink htmlFor="uname">
                                                                    ...
                                                                    orientation? <span className="text-red"> * </span>
                                                                </InputLabel>
                                                                {/* <Select
                                                                    options={options}
                                                                    components={{ Input: CustomInput }}
                                                        ...
                                                        <div className='w-100'>
                                                            <FormControl variant="standard">
                                                                <InputLabel shrink htmlFor="uname">
                                                                    ...
                                                                    Disability Discrimination Act 2005? <span className="text-red"> * </span>
                                                                </InputLabel>
                                                                {/* <Select
                                                                    options={options}
                                                                    components={{ Input: CustomInput }}
                                                    ...
                                                    <div className="col-lg-4 col-md-6 mt-auto select-down-icon">
                                                        <div className='w-100'>
                                                            <FormControl variant="standard" className={preferSelected?.title == "Yes" ? "" : "input-disabled"}>
                                                                <InputLabel shrink htmlFor="uname">
                                                                    If Yes, please specify <span className="text-red"> * </span>
                                                                </InputLabel>
                                                                <TextField disabled={preferSelected?.title !== "Yes"} id="lineManage" placeholder='Please list any disabilities' name='disability_specific' value={forminputData.disability_specific} onChange={inputData} />
                                                    ...
                                                    <div className="col-lg-4 col-md-6 mt-auto select-down-icon">
                                                        <div className='w-100'>
                                                            <FormControl variant="standard">
                                                                <InputLabel shrink htmlFor="uname">
                                                                    Are you the first generation from your family to
                                                                    receive a higher education? <span className="text-red"> * </span>
                                                                </InputLabel>
                                                                {/* <Select
                                                                    options={options}
                                                                    components={{ Input: CustomInput }}
                                                        ...
                                                        <div className='w-100'>
                                                            <FormControl variant="standard">
                                                                <InputLabel shrink htmlFor="uname">
                                                                    ...
                                                                    university degree course or equivalent? <span className="text-red"> * </span>
                                                                </InputLabel>
                                                                {/* <Select
                                                                    options={options}
                                                                    components={{ Input: CustomInput }}
                                                        ...
                                                        <div className='w-100'>
                                                            <FormControl variant="standard">
                                                                <InputLabel shrink htmlFor="uname">
                                                                    ...
                                                                    or your household received income support? <span className="text-red"> * </span>
                                                                </InputLabel>
                                                                {/* <Select
                                                                    options={options}
                                                                    components={{ Input: CustomInput }}
                                                    ...
                                                    <div className="col-lg-4 col-md-6 mt-auto select-down-icon">
                                                        <div className='w-100'>
                                                            <FormControl variant="standard">
                                                                <InputLabel shrink htmlFor="uname">
                                                                    What was the occupation of your main
                                                                    household earner when you were aged about
                                                                    14? <span className="text-red"> * </span>
                                                                </InputLabel>
                                                                {/* <Select
                                                                    options={options}
                                                                    components={{ Input: CustomInput }}
                                                        ...
                                                        <div className='w-100'>
                                                            <FormControl variant="standard">
                                                                <InputLabel shrink htmlFor="uname">
                                                                    ...
                                                                    of 11-18)? <span className="text-red"> * </span>
                                                                </InputLabel>
                                                                {/* <Select
                                                                    options={options}
                                                                    components={{ Input: CustomInput }}
                                                    ...
                                                    <div className="col-lg-4 col-md-6 mt-auto select-down-icon">
                                                        <div className='w-100'>
                                                            <FormControl variant="standard">
                                                                <InputLabel shrink htmlFor="uname">
                                                                    Please indicate your Religion/Belief <span className="text-red"> * </span>
                                                                </InputLabel>
                                                                {/* <Select
                                                                    options={options}
                                                                    components={{ Input: CustomInput }}
                                                        ...
                                                        <div className='w-100'>
                                                            <FormControl variant="standard">
                                                                <InputLabel shrink htmlFor="uname">
                                                                    ...
                                                                    sponsorship for an employment Visa? <span className="text-red"> * </span>
                                                                </InputLabel>
                                            ...
                                            <div className='mt-50 text-end px-60'>
                                                ...
                                                {/* <button className="commom-sky-button" onClick={() => { setValue(value + 1) }}>Next</button> */}
                                                <button className="commom-sky-button" onClick={() => { thirdStep() }} >Next</button>

                                            </div>
                                        </div>

                                        {/* =========== Four Tab ====== */}
                                        <div className={toggleState === 4 ? "contents active-contents" : "contents"}>
                                            <div className='data-privacy'>
                                                ...
                                                <div className='form-start px-60 mt-50'>
                                                    ...
                                                    <div className='checkBox d-flex align-items-center'>
                                                        ...
                                                        <p className='content'> Please read our Privacy Policy which is available at <a target="blank" href='/privacypolicy'>www.harriercandidates.com/privacypolicy </a></p>
                                                    </div>
                                                    <div className='checkBox d-flex align-items-center mt-3'>
                                                        ...
                                                        <p className='content'> Please read our Terms of Use which is available at  <a target="blank" href='/termsofuse'>www.harriercandidates.com/termsofuse </a> </p>
                                                    </div>
                                                    <div className='checkBox d-flex align-items-start mt-5'>
                                                        ...
                                                        <p className='content border-0 bg-sky-a'>We encourage all users to sign up to our Harrier Candidates platform. Those who don’t opt in will not be able to gain Guest Access to the platform for salary & skills benchmarking, nor will their profiles be discoverable by Employers in the wider industry.<span className="text-red"> * </span> </p>
                                                    </div>
                                                    ...
                                                    <div className='checkBox d-flex align-items-start mt-3'>
                                                        <Checkbox {...label} onClick={(e) => setHarrierSearch(harrierSearch)} checked disabled />
                                                        <p className='content'> By signing up with us, we will process your data to approach you about roles on behalf of our clients as part of our Harrier Search service. CVs are only shared with clients with your consent on a case by case basis. </p>
                                                    </div>
                                                    {/* <div className='checkBox d-flex align-items-center justify-content-center mt-3'>
                                                        <p className='content'>&#40; Or &#41;</p>
                                                    </div> */}
                                                    <div className='checkBox d-flex align-items-start mt-3'>
                                                        <Checkbox {...label} onClick={(e) => setHarrierCandidate(!harrierCandidate)} />
                                                        <p className='content'>I am happy to share my data with Harrier Candidates, our platform to enable Employers (the wider industry, not just Harrier Search clients) to search a no-names database and request your no-names CV directly for a specific job? CVs are only shared with Employers on a case by case basis.</p>
                                                    </div>
                                                    <div className='checkBox d-flex align-items-start mt-40'>
                                                        <p className='content pl-0 hear-about-us'> How did you hear about us?</p>
                                                    </div>
                                                    <div>
                                                        <>
                                                            {chearAboutUs.map((hearAboutUs) => (
                                                                <div className='checkBox d-flex align-items-center mt-11'>
                                                                    <Checkbox value={hearAboutUs.id} checked={ckeck == hearAboutUs.id} onChange={(e) => checkedFun(e)} className="channel_check" />
                                                                    <p className='about-us-here'> {hearAboutUs.title}</p>
                                                                </div>
                                                            )
                                                            )}
                                                        </>
                                                        ...
                                                        <div className='specify-input mt-17'>
                                                            {/* <BootstrapInput id=""   /> */}
                                                            <TextField placeholder='If Event or Other, please specify' disabled={ckeck == 5 || ckeck == 6 || ckeck == 7 ? "" : "disabled"} name='otherSpecify' value={forminputData.otherSpecify} onChange={inputData} />
                                                        </div>
                                                        <div className='specify-input'>
                                                            <FormControl variant="standard">
                                                                <InputLabel shrink htmlFor="qualified">
                                                                    If you were referred by an individual, please tell us who so that they’re
                                                                    eligible for a potential referral fee.
                                                                </InputLabel>
                                                                <TextField placeholder='Enter referral name' name='referral' value={forminputData.referral} onChange={inputData} />

                                                            </FormControl>
                                                        </div>

                                                        <div className='specify-input'>
                                                            <FormControl variant="standard" className='mt-1 position-relative'>
                                                                <InputLabel shrink htmlFor="">
                                                                    Upload CV
                                                                    {/* <span className="text-red"> *  </span> */}
                                                                </InputLabel>
                                                                <InputLabel shrink htmlFor="file_upload" className='file_upload d-flex justify-content-center mb-5'>
                                                                    <img src={uploadIcon} alt="" /> <p className='content'>Drag & drop or <span> browse</span></p>
                                                                </InputLabel>
                                                                <TextField name="logo" value="" type="file" className="inputFile" id="file_upload" onChange={handleCvChange}></TextField>
                                                                <div className='d-flex justify-content-between'>
                                                                    {cv?.name}
                                                                    {cv?.name && <button className='bg-transparent border-0 text-right' onClick={() => setCv({})}> <img src={closeIcon} alt="" /> </button>
                                                                    }
                                                                </div>
                                                            </FormControl>
                                                        </div>
                                            ...
                                            >
                                                ...
                                                <Box sx={style}>
                                                    <div className='text-end' >
                                                        <button className='bg-transparent border-0 text-right close-width' onClick={successhandleClose}> <img src={closeIcon} alt="" /> </button>
                                                    </div>
                                                    ...
                                                    <Typography id="success-modal">
                                                        <h5 className='text-center pt-5'>
                                                            Thank you for creating a profile in Harrier Candidates.
                                                        </h5>
                                                        <h6 className='pt-2'>
                                                            You will be contacted by our Candidate Success team if an employer requests to see your anonymised CV regarding a specific vacancy. N.B.: Employer Users will not be able to see the profiles of their own Employees within the database, safeguarding your privacy.
                                                        </h6>
                                                        <h6 className='pt-2'>
                                                            In the meantime, if you would like to browse the Harrier Candidates database for salary benchmarking, at the Log In page you can request Guest Access and our team will issue you with a password that provides 24 hour access.
                                                        </h6>
                                                        <h6 className='pt-2'>
                                                            We’d also like to tell you about our Referral Scheme: every time a Candidate who you referred lands a new role via the platform, you will receive 1% of their new base salary as a thank you from us. Every candidate that signs up helps to create a fuller picture of the industry for everyone, so do please share this link to <Link to=""> www.harriercandidates.com </Link> with your friends and colleagues in LegalTech/Legal Ops.
                                                        </h6>
                                                        <h6 className='pt-2'>
                                                            Finally, we will endeavour to be in touch with you from time to time to help keep your data up to date. If you want to update your profile, or perhaps change your Jobseeker Status (e.g. from Very Passive to Active), do contact our Candidate Success team at <Link to="">cs@harriercandidates.com </Link>
                                                        </h6>
                                                    </Typography>
                                                    <div className='text-center mt-40'>
                                                        <button className="commom-blue-button" onClick={successhandleClose}>Ok</button>
                                                    </div>
                                                </Box>
