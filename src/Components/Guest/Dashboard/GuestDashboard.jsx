...
const GuestDashboard = () => {
  ...
  const [sendSearchResponse2, setSendSearchResponse2] = useState({
    first: "",
    second: "",
  });
  const [secondOption2, setSecondOption2] = useState("");

  let SubOption2 = QuickSearchOptions2.filter((x) =>
    QuickSearch2.includes(x?.select)
  );
  ...
  const quickTypeSearch = (e) => {
    if (e.target.value) {
      const searchData = oldCandidatesListDetails.filter((i) => {
        ...
        let tech_tools_list = i?.tech_tools.map((d, i) =>
  ...
  const getDropDownListTest = async (myFilterData, page) => {
    const encryptedData = Encrypt(
      JSON.stringify({
    ...
    await axiosInstanceAuth
      .post(`/v1/guest/candidates/list?page=${page}`, {
        response: encryptedData,
      })
      .then((res) => {
        const myData = JSON.parse(Decrypt(res?.data?.data));
                      ...
                      {candidatesListDetails.map((d, index) => (
                        ...
                        >
                          ...
                          {selctedColumns.includes("Current Salary") ? (
                            <TableCell component="th" scope="row">
                              {d?.current_salary_symbol_list?.currency_code}{" "}
                              {d.current_salary}
                            </TableCell>
                          ) : null}
                    ...
                    <TableBody>
                      {candidatesListDetails.map((d, index) => (
                        ...
                        >
                          ...
                          <TableCell align="right">
                            {
                              d?.current_bonus_or_commission_symbol_list
                                ?.currency_code
                            }{" "}
                            {d.current_bonus_or_commission}
                          </TableCell>
                          <TableCell align="right">
                            {
                              d?.desired_bonus_or_commission_symbol_list
