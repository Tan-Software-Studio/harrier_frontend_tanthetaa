...
import CorequestionsForm from '../Corequestions/CorequestionsForm';
...
const UpdateGuestProfile = () => {
            ...
            .then((res) => {
                ...
                if (res?.data?.success) {
                    if (finalData?.status == false) {
                        ...
                        setIsShow('active')
                        setUuid(finalData?.uuid)
                        getSingleCandidate(finalData?.uuid, finalData);
                    }
                } else {
                    toast.error(msg);
                }
    ...
    const getSingleCandidate = async (uuid, getData) => {
        setIsLoading(true);
        const encryptedData = Encrypt(
            JSON.stringify({
                uuid: uuid,
        ...
        await axiosInstanceAuth
            ...
            .then((res) => {
                ...
                if (res?.data?.success) {
                    setCandidate(getData);
                    getEmployerTypeListOptions(getData);
                    getCuntryOptions(getData);
                    getDesiredCuntriesOptions(getData);
                    getCurrancyOptions(getData);
                    // getRegionOptions(getData);
                    getWorkingArrangementOptions(getData);
                    getCustomerTypesOptions(getData);
                    getLegalTechToolsOptions(getData);
                    getTechToolsOptions(getData);
                    getQualificationsOptions(getData);
                    getLangaugesOptions(getData);

                    setIndustryTime(getData?.time_in_industry);
                    setCurrentRole(getData?.time_in_current_role);
                }
                // setPqeTime(mydata?.pqe)

                setSendFreelance(freelanceOptions.filter((x) => x.id == getData?.freelance_current)?.[0]);
                setSendFutureFreelance(
                    futureFreelanceOptions.filter((x) => x.id == getData?.freelance_future)?.[0]
                );
                setSendLawDegree(lawDegreeOptions.filter((x) => x.id == getData?.law_degree)?.[0]);
                setSendQualified(qualifiedOptions.filter((x) => x.id == getData?.qualified_lawyer)?.[0]);
                setSendLegalExperience(
                    legalExperienceOptions.filter((x) => x.id == getData?.legal_experience)?.[0]
                );
                setSendLegalTechVendor(
                    legalTechVendorOptions.filter((x) => x.id == getData?.legaltech_vendor_or_consultancy)?.[0]
                );

                setSendHarrierSearch(
                    harrierSearchOptions.filter((x) => x.id == getData?.harrier_search)?.[0]
                );

                setSendHarrierCandidates(
                    harrierCandidatesOptions.filter((x) => x.id == getData?.harrier_candidate)?.[0]
                );
                        ...
                        isLoading ? (<Skeleton
                            ...
                            : <div className='container pt-3'>
                                <div className='d-flex flex-column'>
                                    ...
                                    <CorequestionsForm coreQuestionData={coreQuestionData} />
                                </div>
                            </div>
                        : null
